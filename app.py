...
# ==========================================
    ...
    # with st.form("input_form"):  <-- 이 부분이 사라집니다.
    ...
    st.write("") # 버튼 위 여백 조금 추가
    
    # st.form_submit_button 대신 일반 st.button 사용
    if st.button("데이터 추가", type="primary"):
        st.session_state.df_data = pd.concat([st.session_state.df_data, pd.DataFrame([new_row])], ignore_index=True)
        st.success("데이터가 저장되었습니다.")
        st.rerun()

    # 빠른 데이터 삭제 버튼 (방금 넣은 데이터 지우기)
    ...
    st.caption("표 안의 셀을 더블클릭해 오타를 직접 수정할 수 있습니다. 명백한 에러 데이터는 삭제하는 대신 좌측 '학습 적용' 체크를 해제하세요.")
    
    # hide_index=True로 넘버링 숨김, num_rows="dynamic" 삭제로 + 버튼 차단
    ...
    with c2:
        ...
        if st.button("계산 실행", type="primary"):
            if len(valid_df) < 2:
                ...
                # [개선] 이전 추천값과 비교하여 동일한지 확인
                if "prev_next_points" in st.session_state and st.session_state.prev_next_points == next_points:
                    st.info("💡 **AI의 판단:** 현재 데이터 분포상 이 영역이 최적점일 확률이 가장 높습니다. 추천 조건이 동일하게 유지되고 있습니다.")
